...
from utils import set_seed, EarlyStopping, save_info_opt, save_info_train_valid, save_info_test
from data_loader import GraphDataset, collate
from trainer import train, evaluate
from model import BDGKT
...
train_root = f'./gdata/{opt.dataset}/{opt.cv_num}/{opt.dataset}_{opt.question_max_length}_{opt.student_max_length}/train/'
val_root = f'./gdata/{opt.dataset}/{opt.cv_num}/{opt.dataset}_{opt.question_max_length}_{opt.student_max_length}/val/'
test_root = f'./gdata/{opt.dataset}/{opt.cv_num}/{opt.dataset}_{opt.question_max_length}_{opt.student_max_length}/test/'

train_set = GraphDataset(train_root, load_graphs)
test_set = GraphDataset(test_root, load_graphs)
val_set = GraphDataset(val_root, load_graphs)
...
model = BDGKT(student_num = opt.student_num, question_num = opt.question_num, skill_num = opt.skill_num, input_dim=opt.hidden_size,
             question_max_length=opt.question_max_length, student_max_length=opt.student_max_length,
             drop1=opt.drop1, drop2=opt.drop2,
...
save_info_opt(opt.dataset, opt.batch_size, opt.hidden_size, opt.lr, opt.l2, opt.drop1, opt.drop2,
            opt.layer_num, opt.question_max_length, opt.student_max_length, opt.patience, opt.gpu, opt.cv_num, opt.seed,
            info_file)
