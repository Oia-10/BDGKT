...
def train(model, train_data, optimizer, device):
    ...
    for batch_graph, tstudent_id, tquestion_id, target_res, student, target_question, target_question_exist in train_data:

        optimizer.zero_grad()

        p  = model(batch_graph.to(device),
                    tstudent_id.to(device),
                    tquestion_id.to(device),
                    student.to(device),
                    target_question.to(device),
                    target_question_exist.to(device),
                    )
...
def evaluate(model, val_data, device):
    ...
    with torch.no_grad():
        for  batch_graph, tstudent_id, tquestion_id, target_res, student, target_question, target_question_exist in val_data:
            p = model(batch_graph.to(device),
                    tstudent_id.to(device),
                    tquestion_id.to(device),
                    student.to(device),
