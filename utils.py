...
def save_info_opt(dataset, batch_size, hidden_size, lr, l2, feat_drop, attn_drop, p_drop,
                  layer_num, item_max_length, user_max_length, patience, gpu, cv_num,seed,
                  info_file):
    params_list = (
        'dataset = %s\n' % dataset,
        'batch_size = %d\n' % batch_size,
    ...
    info_file.write('%s %s %s %s %s %s %s %s %s %s %s %s %s %s %s ' % params_list)
...
def save_info_train_valid(cv, epoch, max_auc,
    ...
    print('%s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s' % print_list)
